...
export default function Confirm({ pack }: ConfirmPopProps) {
	...
	onMount(() => {
		...
		onCleanup(() => {
			registerHotkeyEvent("Accept Confirm Message", () => {});
			registerHotkeyEvent("Refuse Confirm Message", () => {});
		});
	});
	return (
		<div class="absolute z-90 flex size-full items-center justify-center backdrop-blur-sm">
			<div class="bg-secondary popup-shadow pointer-events-auto h-fit w-fit max-w-[90%] rounded-xl p-1 text-gray-950 md:max-w-[60%] dark:text-gray-200">
				<div class="border-secondary-10 rounded-xl border border-dashed p-9">
					<h1 class="mb-3 text-3xl font-bold capitalize">
						{pack.title}
					</h1>
