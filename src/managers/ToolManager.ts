...
import { ProfileMain } from "@coretools/Profile/ProfileMain";
...
const CORE_TOOLS = ["profile", "image", "notebook", "kanban", "embed"];
...
export class ToolManager {
        ...
        private async initializeTools(loadUnsigned?: boolean): Promise<void> {
                ...
                this.tools = await Promise.all(
                        this.hand.map(async (tool) => {
                                const toolInstance =
                                        await this.createToolInstance(tool);
                                if (toolInstance) {
                                        this.toolMap.set(
                                                tool.name,
                                                toolInstance,
        ...
        async uninstallTool(title: string): Promise<boolean> {
                ...
                if (tool) {
                        await Promise.all(
                                tool.cards.map((card) =>
                                        this.deleteCard(card.name, name),
                                ),
                        );
        ...
        async loadCard(name: string, toolName: string): Promise<void> {
                ...
                const { tool: handTool, card } = this.getToolAndCard(
                        toolName,
                        name,
                );
                if (handTool && card) {
        ...
        private updateToolMetadata(
                ...
                this.toolMap
                        .get(toolName)
                        ?.toolEvent.emit("metadata", metadata);
        }
