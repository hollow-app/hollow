...
import { useNotifications, NotificationsProps } from "./hooks";

const Notifications: Component<NotificationsProps> = (props) => {
	const { state, actions } = useNotifications(props);

	return (
		<div class="w-ful box-border flex h-full flex-col gap-3 px-3 py-5">
			<div class="flex items-center justify-between border-b border-[var(--high)] pb-4">
				<div class="space-y-1">
				...
				>
					<For each={state.notifications()}>
						{(noti) => {
							...
							return (
								<div class="border-secondary-15/60 pointer-events-auto w-full rounded-xl border bg-[var(--front)] px-3 py-2.5 transition">
