	...
	const driverObj = driver({
		...
		steps: [
			...
			},
			{
				...
				popover: {
					title: "Tool List",
					description: "Pick the tool this card will be based on.",
					side: "right",
					showButtons: ["next"],
				},
			...
			},
			{
				...
				popover: {
					title: "Emoji Picker",
					description: "Choose an emoji that best matches this tool.",
					side: "right",
					showButtons: ["next"],
					onNextClick: () => {
						if (!lastByClass("emoji-picker")) {
			...
			},
			{
				...
				onHighlightStarted: () => {
					const interval = setInterval(() => {
						if (document.getElementById("vault-import-url-btn")) {
							clearInterval(interval);
							driverObj.moveNext();
						}
					}, 100);
