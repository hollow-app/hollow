...
export const useVault = (props: VaultProps): VaultHook => {
    ...
    const [selectedItem, setSelectedItem] = createSignal<VaultItem | null>(null);
    ...
    const importImages = async () => {
        ...
        if (selected) {
            await manager.vault.addItems(selected);
            setImages(manager.vault.getVault());
        }
    };
    ...
    const editItem = () => {
        ...
        const save = async (data: any) => {
            ...
            setSelectedItem((prev) => (prev ? { ...prev, ...wantedData } : null));
        };
        submitForm(save, url);
    };
    ...
    const removeItem = () => {
        ...
        const pack: ConfirmType = {
            ...
            onAccept: () => {
                ...
                manager.vault.removeItems([item.path]);
